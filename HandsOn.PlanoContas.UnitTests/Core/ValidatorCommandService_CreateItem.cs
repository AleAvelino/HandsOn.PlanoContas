...
namespace HandsOn.PlanoContas.UnitTests.Core
{
    public class ValidatorCommandService_CreateItem
    {
        ...
        public void IsAcceptHasNotChildren_InputIsTrue_ReturnFalse()
        {
            ...
            bool result = _validatorCommandService.ParentCanHaveChildren(item);
        ...
        [InlineData("2.3")]
        [InlineData("4.1")]
        public void CodeAlreadyExists_ValuesRepeated_ReturnTrue(string value)
        {
            ChartAccount item = new(0, value, $"Testes de Repetição de código {value}", true, PlanoContas.Core.Enums.EPlanType.Expense, "2.3", 0);
            bool result = _validatorCommandService.CodeAlreadyExists(item);

            Assert.True(result, $"O Código {value} já existe");
        }
        [Theory]
        [InlineData("2.5")]
        [InlineData("3.9")]
        [InlineData("5")]
        public void CodeAlreadyExists_ValuesNotRepeated_ReturnFalse(string value)
        {
            ChartAccount item = new(0, value, $"Testes de Repetição de código {value}", true, PlanoContas.Core.Enums.EPlanType.Expense, "2.3", 0);
            bool result = _validatorCommandService.CodeAlreadyExists(item);

            Assert.False(result, $"O Código {value} não se repetiu");
        }
        ...
        public void ChildrenMustBeSameParentType_DifferentValues_ReturnFalse(string code, string fatherCode, PlanoContas.Core.Enums.EPlanType value)
        {
            ChartAccount item = new(0, code, $"Testes de tipo do pai {value}", true, value, fatherCode, 0);
            bool result = _validatorCommandService.ChildrenMustBeSameParentType(item);
        ...
        public void ChildrenMustBeSameParentType_SamesValues_ReturnTrue(string code, string fatherCode, PlanoContas.Core.Enums.EPlanType value)
        {
            ChartAccount item = new(0, code, $"Testes de tipo do pai {value}", true, value, fatherCode, 0);
            bool result = _validatorCommandService.ChildrenMustBeSameParentType(item);
        ...
        [InlineData("6", "4", PlanoContas.Core.Enums.EPlanType.Expense)]
        public void CodeCanBeGreaterThanNext_ValuesBigger_ReturnTrue(string value, string fatherCode, PlanoContas.Core.Enums.EPlanType type)
        {
            ...
            Assert.True(result, $"O Código {value} é maior do que o último");
        }
